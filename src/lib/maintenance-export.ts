...
export function printMaintenanceReport(report: ReportData) {
  ...
  </style></head><body><header><div><div class="brand">مصباحة لإدارة صيانة الأسطول</div><h1>${escapeHtml(report.title)}</h1><p>${escapeHtml(report.subtitle || "تقرير عمليات الصيانة والتكاليف")}</p></div><div>${new Date().toLocaleString("ar-SA")}</div></header>${assetDetails}<section class="summary"><div><span>عدد عمليات الصيانة</span><strong>${report.operations.length.toLocaleString("ar-SA")}</strong></div>${report.monthlyCost === undefined ? "" : `<div><span>تكلفة الشهر</span><strong>${formatMoney(report.monthlyCost)}</strong></div>`}${report.yearlyCost === undefined ? "" : `<div><span>تكلفة السنة</span><strong>${formatMoney(report.yearlyCost)}</strong></div>`}<div class="accent"><span>إجمالي التكاليف</span><strong>${formatMoney(report.totalCost)}</strong></div></section><table><thead><tr><th>م</th><th>رقم الأصل</th><th>عملية الصيانة</th><th>التاريخ</th><th>العداد</th><th>مركز الصيانة</th><th>رقم الفاتورة</th><th>التكلفة</th></tr></thead><tbody>${rows || '<tr><td colspan="8">لا توجد عمليات صيانة</td></tr>'}</tbody><tfoot><tr><td colspan="7">الإجمالي</td><td class="money">${formatMoney(report.totalCost)}</td></tr></tfoot></table><footer>تم إنشاء التقرير بواسطة نظام مصباحة لإدارة صيانة الأسطول</footer></body></html>`;
